...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const currentLabel = navItems.find((i) => i.href === pathname)?.label || "대시보드";
  ...
  return (
    <div className="flex h-screen bg-gray-50">
      ...
      <MobileSidebarOverlay isOpen={sidebarOpen} onClose={closeSidebar}>
        <SidebarContent user={user} pathname={pathname} onLogout={handleLogout} />
      </MobileSidebarOverlay>
      ...
      <div className="flex-1 flex flex-col overflow-hidden min-w-0">
        ...
        <header className="bg-white border-b border-gray-200 px-4 lg:px-6 py-3 flex items-center justify-between shrink-0">
          ...
          <div className="flex items-center gap-2 lg:gap-4">
            ...
            <Link
              href="/admin"
              className="hidden sm:inline-flex text-xs text-gray-400 hover:text-blue-600 transition border border-gray-200 px-3 py-1.5 rounded-lg"
            >
              어드민
            </Link>
          </div>
